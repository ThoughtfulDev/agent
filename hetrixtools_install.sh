...
mkdir -p /root/hetrixtools
echo "... done."

# Fetching new agent
echo "Fetching the new agent..."
wget -t 1 -T 30 -qO /root/hetrixtools/hetrixtools_agent.sh --no-check-certificate https://raw.githubusercontent.com/ThoughtfulDev/agent/master/hetrixtools_agent.sh
echo "... done."

# Inserting Server ID (SID) into the agent config
echo "Inserting Server ID (SID) into agent config..."
sed -i "s/SIDPLACEHOLDER/$SID/" /root/hetrixtools/hetrixtools_agent.sh
echo "... done."
...
then
	...
	sed -i "s/CheckServices=\"\"/CheckServices=\"$3\"/" /root/hetrixtools/hetrixtools_agent.sh
fi
echo "... done."
...
then
	...
	sed -i "s/CheckSoftRAID=0/CheckSoftRAID=1/" /root/hetrixtools/hetrixtools_agent.sh
fi
echo "... done."
...
then
	...
	sed -i "s/CheckDriveHealth=0/CheckDriveHealth=1/" /root/hetrixtools/hetrixtools_agent.sh
fi
echo "... done."
...
then
	...
	sed -i "s/RunningProcesses=0/RunningProcesses=1/" /root/hetrixtools/hetrixtools_agent.sh
fi
echo "... done."
...
then
	...
	sed -i "s/ConnectionPorts=\"\"/ConnectionPorts=\"$7\"/" /root/hetrixtools/hetrixtools_agent.sh
fi
echo "... done."
...
bash /root/hetrixtools/hetrixtools_agent.sh > /dev/null 2>&1 &
echo "... done."
